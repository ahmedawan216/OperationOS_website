...
export default async function RecruitOSAnalysisResultPage({ params, searchParams }: PageProps) {
  ...
  if (!jobId) {
    return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap"><Link href="/dashboard/agents/recruitos/jobs" className="mb-10 inline-flex items-center gap-2 text-[13px] text-ink-dim"><span>←</span> Back to Jobs</Link><div className="rounded-xl border border-border p-6 sm:p-8"><h1 className="font-display text-xl font-semibold text-ink">Job not specified</h1><p className="mt-3 text-sm text-ink-dim">This analysis does not have a job associated with it.</p></div></div></main>;
  }
  ...
  if (analysisError || jobError || !analysis || !job) {
    console.error("Analysis result error:", analysisError, jobError);
    return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap"><Link href={`/dashboard/agents/recruitos/jobs/${jobId}`} className="mb-10 inline-flex items-center gap-2 text-[13px] text-ink-dim"><span>←</span> Back to Job</Link><div className="rounded-xl border border-border p-6 sm:p-8"><h1 className="font-display text-xl font-semibold text-ink">Analysis not found</h1><p className="mt-3 text-sm leading-[1.6] text-ink-dim">RecruitOS could not find the analysis for this candidate and job.</p></div></div></main>;
  }
  ...
  return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap">
    <Link href={`/dashboard/agents/recruitos/jobs/${jobId}`} className="mb-10 inline-flex items-center gap-2 text-[13px] text-ink-dim"><span>←</span> Back to Job</Link>
    <div className="mb-12"><p className="mb-4 font-mono text-[11px] tracking-[0.08em] text-accent">RECRUITOS / ANALYSIS RESULT</p><h1 className="font-display text-[clamp(36px,5vw,64px)] font-semibold leading-[1.05] tracking-[-0.025em] text-ink">Candidate analysis.</h1><p className="mt-5 max-w-[700px] text-[15px] leading-[1.7] text-ink-dim">RecruitOS analyzed this candidate against the selected job.</p></div>
    ...
    <div className="mt-8 flex flex-wrap gap-3"><Link href={`/dashboard/agents/recruitos/analyze?jobId=${jobId}`} className="rounded-lg bg-accent px-5 py-3 text-sm font-medium text-bg transition-opacity hover:opacity-90">Analyze another candidate</Link><Link href={`/dashboard/agents/recruitos/jobs/${jobId}`} className="rounded-lg border border-border px-5 py-3 text-sm font-medium text-ink transition-colors hover:border-accent hover:text-accent">Back to Job</Link><Link href="/dashboard/agents/recruitos/jobs" className="rounded-lg border border-border px-5 py-3 text-sm font-medium text-ink transition-colors hover:border-accent hover:text-accent">All Jobs</Link></div>
  </div></main>;
}
