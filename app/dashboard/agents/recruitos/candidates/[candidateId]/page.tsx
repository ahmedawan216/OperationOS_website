...
export default async function CandidateProfilePage({ params, searchParams }: Props) {
  ...
  return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap">
    ...
    <section className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><div className="space-y-5">
      ...
      <section className="rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-accent">EDUCATION</p><div className="mt-6 space-y-4">{education.length ? education.map((item, index) => <div key={`${item.institution}-${index}`}><h2 className="font-display text-lg font-semibold text-ink">{item.degree || "Education"}{item.field ? ` · ${item.field}` : ""}</h2><p className="mt-1 text-sm text-ink-dim">{item.institution || "Institution"}</p></div>) : <p className="text-sm text-ink-dim">No structured education extracted.</p>}</div></section>
    </div>

    <aside className="space-y-5">
      <section className="rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-accent">JOB MATCHES</p><div className="mt-5 space-y-3">{matches?.length ? matches.map((match) => { const job = Array.isArray(match.jobs) ? match.jobs[0] : match.jobs; return <Link key={match.id} href={`/dashboard/agents/recruitos/candidates/${candidateId}?jobId=${match.job_id}`} className={`block rounded-lg border p-4 ${selectedMatch?.id === match.id ? "border-accent/50" : "border-border"}`}><div className="flex items-center justify-between gap-3"><span className="truncate text-sm font-medium text-ink">{job?.title || "Job"}</span><span className="font-mono text-sm text-accent">{match.match_score ?? "—"}</span></div><p className="mt-2 text-xs capitalize text-ink-dim">{match.recruiter_status} · {match.recommendation ?? "pending"}</p></Link> }) : <p className="text-sm text-ink-dim">No jobs associated yet.</p>}</div></section>

      {analysis && <section className="rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-accent">AI INTELLIGENCE</p><p className="mt-4 font-display text-4xl font-semibold text-ink">{String(analysis.match_score)}/100</p><p className="mt-2 text-xs capitalize text-ink-dim">{String(analysis.recommendation)} · {String(analysis.confidence_level)} confidence</p><p className="mt-5 text-sm leading-[1.7] text-ink-dim">{String(analysis.summary)}</p><div className="mt-6 space-y-4"><div><p className="text-xs font-medium text-ink">Why this candidate matches</p><p className="mt-2 text-sm leading-[1.7] text-ink-dim">{String(analysis.why_strong_match)}</p></div><div><p className="text-xs font-medium text-ink">Strengths</p><div className="mt-2 flex flex-wrap gap-2">{(Array.isArray(analysis.matching_skills) ? analysis.matching_skills : []).map((skill) => <span key={String(skill)} className="rounded-md border border-border px-2.5 py-1.5 text-xs text-ink">{String(skill)}</span>)}</div></div><div><p className="text-xs font-medium text-ink">Gaps</p><div className="mt-2 space-y-1 text-sm text-ink-dim">{(Array.isArray(analysis.missing_skills) ? analysis.missing_skills : []).map((skill) => <p key={String(skill)}>• {String(skill)}</p>)}</div></div><div><p className="text-xs font-medium text-ink">Concerns</p><div className="mt-2 space-y-1 text-sm text-ink-dim">{(Array.isArray(analysis.potential_concerns) ? analysis.potential_concerns : []).map((item) => <p key={String(item)}>• {String(item)}</p>)}</div></div></div></section>}

      <section className="rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-accent">RESUMES</p><div className="mt-5 space-y-3">{resumes?.length ? resumes.map((resume) => <div key={resume.id} className="rounded-lg border border-border p-4"><p className="truncate text-sm text-ink">{resume.original_filename}</p><p className="mt-1 text-xs capitalize text-ink-dim">{resume.processing_status || resume.status}</p></div>) : <p className="text-sm text-ink-dim">No resumes attached.</p>}</div></section>
    </aside></section>

    <section className="mt-5 rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-accent">PRIVATE NOTES</p><div className="mt-6"><CandidateNoteForm candidateId={candidateId} /></div><div className="mt-8 space-y-4">{notes?.map((note) => <div key={note.id} className="border-t border-border pt-4"><p className="text-sm leading-[1.7] text-ink">{note.content}</p><p className="mt-2 text-xs text-ink-dim">{new Date(note.created_at).toLocaleString()}</p></div>)}</div></section>

    <section className="mt-5 rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-accent">ACTIVITY</p><div className="mt-6 space-y-4">{activity?.length ? activity.map((item) => <div key={item.id} className="flex gap-4 border-l border-border pl-4"><div><p className="text-sm text-ink">{item.type.replaceAll("_", " ")}</p><p className="mt-1 text-xs text-ink-dim">{new Date(item.created_at).toLocaleString()}</p></div></div>) : <p className="text-sm text-ink-dim">No activity recorded yet.</p>}</div></section>
  </div></main>;
}
