...
export default async function RecruitOSCandidatesPage({ searchParams }: Props) {
  ...
  // A candidate is a first-class record as soon as a resume is successfully
  // extracted. Job analysis is a separate step, so job filtering must not
  // depend exclusively on candidate_job_matches.
  let candidateIds: string[] | null = null;

  if (params.jobId) {
    const [{ data: jobActivities, error: activityFilterError }, { data: jobMatches, error: matchFilterError }] = await Promise.all([
      supabaseAdmin
    ...
    if (activityFilterError) console.error("Candidate job activity filter error:", activityFilterError);
    if (matchFilterError) console.error("Candidate job match filter error:", matchFilterError);

    const jobCandidateIds = [
      ...(jobActivities ?? []).map((row) => row.candidate_id),
      ...(jobMatches ?? []).map((row) => row.candidate_id),
    ];
    candidateIds = [...new Set(jobCandidateIds)];
  }

  if (params.status && statuses.includes(params.status) || params.recommendation && recommendations.includes(params.recommendation)) {
    ...
    const filteredIds = [...new Set((matchingIds ?? []).map((row) => row.candidate_id))];
    candidateIds = candidateIds ? candidateIds.filter((id) => filteredIds.includes(id)) : filteredIds;
  }
  ...
  if (candidateIds) {
    candidatesQuery = candidateIds.length
      ? candidatesQuery.in("id", candidateIds)
      : candidatesQuery.in("id", ["00000000-0000-0000-0000-000000000000"]);
  }
  if (q) {
