...
export default async function RecruitOSCandidatesPage({ searchParams }: Props) {
  ...
  return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap">
    ...
    <form className="rounded-xl border border-border p-5 sm:p-6"><div className="grid gap-3 md:grid-cols-[1.5fr_1fr_1fr_1fr_auto]"><input name="q" defaultValue={q} placeholder="Search name or email" className="rounded-lg border border-border bg-transparent px-4 py-3 text-sm text-ink outline-none placeholder:text-ink-dim focus:border-accent" /><select name="jobId" defaultValue={params.jobId ?? ""} className="rounded-lg border border-border bg-bg px-4 py-3 text-sm text-ink outline-none"><option value="">All jobs</option>{jobs?.map((job) => <option key={job.id} value={job.id}>{job.title}</option>)}</select><select name="status" defaultValue={params.status ?? ""} className="rounded-lg border border-border bg-bg px-4 py-3 text-sm text-ink outline-none"><option value="">All stages</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select><select name="recommendation" defaultValue={params.recommendation ?? ""} className="rounded-lg border border-border bg-bg px-4 py-3 text-sm text-ink outline-none"><option value="">All recommendations</option>{recommendations.map((item) => <option key={item} value={item}>{item}</option>)}</select><button className="rounded-lg bg-accent px-5 py-3 text-sm font-medium text-bg">Search</button></div><div className="mt-4 flex flex-wrap items-center gap-2"><span className="text-xs text-ink-dim">Sort:</span>{[["score", "Match score"], ["newest", "Newest"], ["oldest", "Oldest"]].map(([value, label]) => <Link key={value} href={href({ sort: value, page: "1" })} className={`rounded-md border px-3 py-1.5 text-xs ${sort === value ? "border-accent text-accent" : "border-border text-ink-dim"}`}>{label}</Link>)}</div></form>
