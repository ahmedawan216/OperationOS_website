...
export default function CandidatePipeline({ candidates, jobId }: Props) {
  ...
  return <section className="mt-5 rounded-xl border border-border p-6 sm:p-8">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-[11px] tracking-[0.08em] text-accent">CANDIDATE PIPELINE</p><h2 className="mt-3 font-display text-2xl font-semibold text-ink">Candidates</h2><p className="mt-2 text-sm leading-[1.6] text-ink-dim">Ranked by stored match score. Move each candidate through the job-specific recruiting pipeline.</p></div><div className="flex flex-wrap gap-2">{selected.length >= 2 && <Link href={compareHref} className="inline-flex w-fit rounded-lg border border-accent px-5 py-3 text-sm font-medium text-accent">Compare ({selected.length})</Link>}</div></div>
