...
function candidateName(candidate: Candidate) {
  const value = Array.isArray(candidate.candidates) ? candidate.candidates[0] : candidate.candidates;
  return value?.full_name || value?.email || "Unnamed candidate";
}
...
export default function CandidatePipeline({ candidates, jobId }: Props) {
  ...
  return <section className="mt-5 rounded-xl border border-border p-6 sm:p-8">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-[11px] tracking-[0.08em] text-accent">CANDIDATE PIPELINE</p><h2 className="mt-3 font-display text-2xl font-semibold text-ink">Candidates</h2><p className="mt-2 text-sm leading-[1.6] text-ink-dim">Ranked by stored match score. Move each candidate through the job-specific recruiting pipeline.</p></div><Link href={`/dashboard/agents/recruitos/analyze?jobId=${jobId}`} className="inline-flex w-fit rounded-lg bg-accent px-5 py-3 text-sm font-medium text-bg">Analyze candidate</Link></div>
