...
export default async function RecruitOSJobPage({ params }: PageProps) {
  ...
  if (jobError || !job) {
    return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap"><Link href="/dashboard/agents/recruitos/jobs" className="mb-10 inline-flex items-center gap-2 text-[13px] text-ink-dim"><span>←</span> Back to Jobs</Link><div className="rounded-xl border border-border p-8"><h1 className="font-display text-xl font-semibold text-ink">Job not found</h1><p className="mt-3 text-sm text-ink-dim">This job could not be found.</p></div></div></main>;
  }

  const { data: candidates, error: candidatesError } = await supabaseAdmin
    .from("candidate_job_matches")
    .select("id, candidate_id, resume_id, recommendation, match_score, recruiter_status, created_at, candidates ( id, full_name, email, headline )")
    .eq("job_id", jobId)
    .eq("user_id", user.id)
