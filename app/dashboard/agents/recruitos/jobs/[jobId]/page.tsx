...
export default async function RecruitOSJobPage({ params }: PageProps) {
  ...
  const { data: candidates, error: candidatesError } = await supabaseAdmin.from("candidate_job_matches").select("id, candidate_id, resume_id, recommendation, match_score, recruiter_status, created_at, candidates ( id, full_name, email, headline )").eq("job_id", jobId).eq("user_id", user.id).order("match_score", { ascending: false, nullsFirst: false });
  ...
  return <main className="min-h-screen bg-bg px-5 pb-20 pt-28 sm:px-8"><div className="mx-auto max-w-wrap">
    <Link href="/dashboard/agents/recruitos/jobs" className="mb-10 inline-flex items-center gap-2 text-[13px] text-ink-dim">← Back to Jobs</Link>
    ...
    <section className="mt-5 rounded-xl border border-border p-6 sm:p-8"><p className="font-mono text-[11px] tracking-[0.08em] text-ink-dim">JOB DESCRIPTION</p><h2 className="mt-4 font-display text-2xl font-semibold text-ink">Requirements</h2><div className="mt-8 rounded-lg border border-border p-6"><p className="whitespace-pre-wrap text-sm leading-[1.8] text-ink-dim">{job.description}</p></div><div className="mt-8 flex flex-wrap gap-3"><Link href={`/dashboard/agents/recruitos/analyze?jobId=${job.id}`} className="rounded-lg border border-border px-5 py-3 text-sm font-medium text-ink hover:border-accent hover:text-accent">Analyze a single resume</Link><Link href="/dashboard/agents/recruitos/jobs" className="rounded-lg border border-border px-5 py-3 text-sm font-medium text-ink">All jobs</Link></div></section>
