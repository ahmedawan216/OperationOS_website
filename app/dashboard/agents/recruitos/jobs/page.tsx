        ...
        <div className="mb-12 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-[700px]">
            ...
            <p className="mt-5 text-[15px] leading-[1.7] text-ink-dim">
              Manage your open roles and analyze candidates
              against the right job.
            </p>
          </div>

          <Link
            href="/dashboard/agents/recruitos/jobs/new"
            className="inline-flex shrink-0 items-center justify-center rounded-lg bg-accent px-5 py-3 text-sm font-medium text-bg transition-opacity hover:opacity-90"
          >
            + New job
          </Link>
        </div>
        ...
        {!error && (!jobs || jobs.length === 0) && (
          <div className="rounded-xl border border-border p-8 text-center sm:p-12">
            ...
            <p className="mt-3 text-sm leading-[1.6] text-ink-dim">
              Create your first job to start analyzing
              candidates with RecruitOS.
            </p>
          ...
          <section className="overflow-hidden rounded-xl border border-border">
            ...
            <div className="divide-y divide-border">
              {jobs.map((job: Job) => (
                <div
                  key={job.id}
                  className="grid gap-5 px-6 py-6 sm:grid-cols-[1fr_120px_150px_160px] sm:items-center sm:gap-6"
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 line-clamp-2 text-sm leading-[1.6] text-ink-dim">
                      {job.description}
                    </p>
                  </div>
                  ...
                  <Link
                    href={`/dashboard/agents/recruitos/jobs/${job.id}`}
                    className="inline-flex w-fit rounded-lg border border-border px-4 py-2.5 text-sm font-medium text-ink transition-colors hover:border-accent hover:text-accent"
                  >
                    Open job →
                  </Link>
                </div>
