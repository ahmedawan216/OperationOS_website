...
export const evaluationSummarySchema = z.object({ productKey:id, evaluationId: id, candidateId: id, baselineVersionId: id, datasetVersionId: id, evaluatorVersionIds: z.array(id).min(1).max(100), status: z.enum(["improved", "regressed", "unchanged", "inconclusive", "invalid"]), metricSummary: z.array(z.object({ key: id, baseline: z.number(), candidate: z.number(), result: id }).strict()).max(100), regressionIds: z.array(id).max(500), riskGateDecision: z.enum(["reject", "inconclusive", "require_human_approval", "canary_eligible"]) }).strict();
export const safetySummarySchema = z.object({ productKey:id, assessmentId: id, candidateId: id, severity: z.enum(["low", "medium", "high"]), guardianDisposition: id, runtimeDisposition: id, findingSummaries: z.array(safeText).max(100), evidenceIds: z.array(id).max(1_000), assessedAt: time }).strict();
export const approvalSummarySchema = z.object({ productKey:id, approvalId: id, actionType: id, subjectId: id, actionDigest: id, payloadSummary: safeText, risk: z.enum(["low", "medium", "high"]), status: z.enum(["pending", "approved", "rejected", "expired", "consumed"]), expiresAt: time, evaluationId: id.optional(), assessmentId: id.optional() }).strict();
export const versionSummarySchema = z.object({ productKey:id, versionId: id, status: z.enum(["known_good", "candidate"]), digest: id, pointer: z.enum(["known_good", "canary", "none"]), createdAt: time }).strict();
export const canarySummarySchema = z.object({ productKey:id, canaryId: id, state: z.enum(["idle", "canary", "rolled_back", "promotion_eligible"]), candidateVersionId: id, knownGoodVersionId: id, rollbackVersionId: id, conditionsDigest: z.string().regex(/^sha256:[a-f0-9]{64}$/), target: z.enum(["test", "preview"]), allocationPercent: z.number().int().min(1).max(10), productionActivationAllowed: z.literal(false), eventSummaries: z.array(safeText).max(500) }).strict();
